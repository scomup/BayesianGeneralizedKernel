...
class Map2D():
    def __init__(self, x_min, x_max, resolution, radius=0.2):
        ...
        self.variance = np.full_like(self.location,0)
    ...
    def update(self, x, y):
        ...
        #idx = int(np.clip( (x)/self.resolution, 0, np.size(self.location)-1))
        #v_cur = self.variance[idx]
        v_cur = np.max(var)
        w = np.exp(-v_cur*100)

        k = self.sparse_kernel(x,x_star,l=0.5*w)
        ...
        """
        result = np.where(x_star == 3.29)
        if(result[0].size!=0):
            idx = result[0][0]
            self.weight.append(k[idx])
            self.value.append(y)
            m = np.average(self.value,weights=self.weight)
            v  = np.average((self.value-m)**2, weights=self.weight)

            self.v = (lam[idx]/(lam_star[idx]))*(self.v + k[idx]*(mu_0[idx] - y)**2/lam_star[idx])
...
for i in range(10000):
    ...
    if(show == 100):
        show = 0
        #variance = sigma**2/map2d.lambdas
        std = np.sqrt(map2d.variance)
        plt.scatter(x, y, c='r')
        plt.plot(map2d.location,map2d.elevation, label='predicted points')
        plt.plot(map2d.location,map2d.variance, label='variance',linestyle="dashed")
        plt.plot(map2d.location,map2d.elevation + 2*std,c='b',linestyle="dotted")
        plt.plot(map2d.location,map2d.elevation - 2*std,c='b',linestyle="dotted", label='3 sigma')
